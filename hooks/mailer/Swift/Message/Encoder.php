...
class Swift_Message_Encoder
{
	 ...
	 */
	var $CHEAP_ADDRESS_RE = '(?#Start of dot-atom
	...
	function &instance()
	{
		...
		if (!$instance)
		{
			$instance = array(new Swift_Message_Encoder());
		}
		return $instance[0];
	}
	/**
	 ...
	 * @return array
	 */
	function addressChunk($input)
	{
		$elements = 0;
		while (preg_match('/^(.*?)(<' . $this->CHEAP_ADDRESS_RE . '>)/s', $input, $matches))
		{
	 ...
	 * @return string
	 */
	function base64Encode($data, $chunk=76, $init_chunk=0, $preserve_address=false, $le="\r\n")
	{
		$ret = "";
		$chunk -= 2;
		...
		if ($preserve_address) $data = $this->addressChunk($data);
	 ...
	 * @return string
	 */
	function rawBase64Encode($string)
	{
	 ...
	 */
	function &base64EncodeFile($file, $chunk=76, $le="\r\n")
	{
	 ...
	 * @return string
	 */
	function QPEncode($data, $chunk=76, $init_chunk=0, $preserve_address=false, $le="\r\n")
	{
		$ret = "";
		if ($preserve_address) $data = $this->addressChunk($data);
		else $data = array($data);
		...
		foreach ($data as $key => $string)
		{
			...
			else
			{
				...
				if ($init_chunk > 3)
				{
					if (preg_match('/^.{1,'.($init_chunk-6).'}([^=]{0,3})?/', $string, $matches, PREG_OFFSET_CAPTURE))
					{
						$ret .= $matches[0][0] . "=";
				...
				preg_match_all('/.{1,'.($chunk-6).'}([^=]{0,3})?/', $string, $matches);
				
				if (count($matches[0])) $ret .= $le . implode("=" . $le, $matches[0]) . "=";
			}
			$init_chunk = 0;
		}
	...
	function rawQPEncode($string)
	{
		...
		{
			$val = ord($string{$i});
			//9, 32 = HT, SP; 33-60 & 62-126 are ok
	 ...
	 * @throws Swift_FileException If the file cannot be read
	 */
	function &QPEncodeFile($file, $chunk=76, $le="\r\n")
	{
		Swift_ClassLoader::load("Swift_CacheFactory");
		if (!is_a($file, "Swift_File"))
		{
	 ...
	 * @throws Swift_FileException If the file cannot be read
	 */
	function &encode7BitFile($file, $chunk=76, $le="\r\n")
	{
		Swift_ClassLoader::load("Swift_CacheFactory");
		if (!is_a($file, "Swift_File"))
		{
	 ...
	 * @return string
	 */
	function header7BitEncode($data, $chunk=76, $init_chunk=0, $le="\r\n")
	{
		$data = $this->encode7BitPrintable($data);
		$ret = "";
		if ($init_chunk > 2)
		{
			$ret .= substr($data, 0, $init_chunk-2) . $le;
			$data = substr($data, $init_chunk-2);
		}
		elseif ($init_chunk) $ret .= $le;
	 ...
	 * @return string
	 */
	function encode7BitPrintable($data)
	{
	 ...
	 * @return string
	 */
	function rfc2047Encode($str, $charset="iso-8859-1", $language="en-us", $chunk=76, $le="\r\n")
	{
		$lang_spec = "";
