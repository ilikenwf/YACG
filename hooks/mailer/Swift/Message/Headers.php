...
class Swift_Message_Headers
{
	 ...
	 * @var array headers
	 */
	var $emailContainingHeaders = array(
	...
	function set($name, $value)
	{
		...
		if (is_array($value))
		{
			foreach ($value as $v)
			{
				if (!$this->getCharset() && $encoder->isUTF8($v))
				{
		...
		elseif ($value !== null)
		{
			if (!$this->getCharset() && $encoder->isUTF8($value))
			{
				$this->setCharset("utf-8");
			}
		}
	 ...
	 */
	function forceEncoding($force=true)
	{
	 ...
	 * @throws Swift_Message_MimeException If no such header exists
	 */
	function setAttribute($header, $name, $value)
	{
	 ...
	 */
	function hasAttribute($header, $name)
	{
		$name = strtolower($name);
		$lheader = strtolower($header);
 		if (!$this->has($header))
	 ...
	 * @throws Swift_Message_MimeException If no such header exists
	 */
	function removeAttribute($header, $name)
	{
		$name = strtolower($name);
		$lheader = strtolower($header);
		if ($this->has($header))
	...
	function getEncoded($name)
	{
		...
		foreach ($encoded_value as $key => $row)
		{
			...
			if ($encoder->is7BitPrintable($row) && !$this->forceEncoding)
			{
				...
				$encoded_value[$key] = $encoder->header7BitEncode(
			...
			elseif ($this->encoding == "Q") //QP encode required
			{
				...
				//If the header is in the list of headers which may have an email address in,
				// preserve the address during encoding
				$save_email = in_array($name, $this->emailContainingHeaders);
	 ...
	 * THIS SERIOUSLY NEEDS REFACTORING
	 * @return string
	 */
	function build()
	{
