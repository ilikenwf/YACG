...
function video($source ='youtube', $keyword = THIS_PAGE_KEYWORD, $width = '400', $height = '300', $params = '') {
  switch ($source) {
    ...
    if (preg_match('/photo_id: \'(.+?)\'/im', $results, $video)) {
      print '<object type="application/x-shockwave-flash" width="'.$width.'" height="'.$height.'" data="http://www.flickr.com/apps/video/stewart.swf?v=1.161"
    ...
    preg_match_all('/<blip:item_id>(\d+)<\/blip:item_id>/im', $results, $videos);
    foreach($videos[1] as $file) {
      $results = fetch('http://blip.tv/file/'.$file);
      if (preg_match('/<meta name=\"video_src\" content=\"http:\/\/blip\.tv\/play\/(.+?)\" \/>/im', $results, $video)) {
        print '<embed src="http://blip.tv/play/'.$video[1].'" type="application/x-shockwave-flash" width="'.$width.'" height="'.$height.'" allowscriptaccess="always" allowfullscreen="true"></embed>';
        return true;
      }
    }
