...
function peakclick($keyword = THIS_PAGE_KEYWORD, $items = 5, $thumbs = true, $bare = false) {
  if (SHOW_ADS) {
    ...
    $ads = fetch('http://feed.peakclick.com/res.php?aff='.PEAKCLICK_AFF.'&subaff='.PEAKCLICK_SUBAFF.'&ref=http://'.THIS_DOMAIN.'&keyword='.urlencode($keyword).'&num='.$items.'&ip='.$ip.'&ua='.urlencode($_SERVER['HTTP_USER_AGENT']).($thumbs ? '&thumbs=1' : ''));
